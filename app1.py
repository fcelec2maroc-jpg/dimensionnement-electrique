...
st.markdown(
    ...
    .stButtonbutton { border-radius 5px; font-weight bold; }
...
class FCELEC_Report(FPDF)
    ...
    def footer(self)
        ...
        self.cell(0, 5, fFC ELEC - WhatsApp  +212 6 74 53 42 64  Page {self.page_no()}, 0, 0, C)
...
if check_password()
    ...
    st.sidebar.download_button(📥 Sauvegarder Projet (.json), data=projet_json, file_name=f{sanitize_text(st.session_state.projet['info']['nom'])}.json, mime=applicationjson)
    ...
    st.sidebar.markdown(---)
    st.sidebar.markdown(h3 style='text-align center; color #FF4B4B;'🎓 FORMATIONS EXPERTh3, unsafe_allow_html=True)
    ...
    st.sidebar.markdown(f
        a href=httpswa.me212674534264 target=_blank style=text-decoration none;
            div style=background-color #25D366; color white; padding 10px; border-radius 5px; text-align center; margin-bottom 8px; font-weight bold; font-size 0.9em; transition 0.3s;🟢 WHATSAPPdiv
        a
        a href=httpswww.linkedin.comcompanyformation-et-consulting-en-electricite-fcelec target=_blank style=text-decoration none;
            div style=background-color #0077B5; color white; padding 10px; border-radius 5px; text-align center; margin-bottom 8px; font-weight bold; font-size 0.9em;🔵 LINKEDINdiv
        a
        a href=httpswww.facebook.comprofile.phpid=61586577760070 target=_blank style=text-decoration none;
            div style=background-color #1877F2; color white; padding 10px; border-radius 5px; text-align center; margin-bottom 8px; font-weight bold; font-size 0.9em;🔵 FACEBOOKdiv
        a
        a href=httpswww.youtube.com@FCELECACADEMY target=_blank style=text-decoration none;
            div style=background-color #FF0000; color white; padding 10px; border-radius 5px; text-align center; margin-bottom 8px; font-weight bold; font-size 0.9em;🔴 YOUTUBEdiv
        a
    , unsafe_allow_html=True)
        ...
        with st.container(border=True)
            ...
            with st.form(ajout_cable)
                ...
                if st.form_submit_button(Calculer et Ajouter au Carnet)
                    ...
                    st.success(fCircuit '{ref_c}' calculé avec succès  {type_cable} {S_ret} mm² protégé par {In}A.)
        ...
        if st.session_state.projet[cables]
            ...
            def generate_pdf_cables()
                ...
                for row in st.session_state.projet[cables]
                    ...
                    pdf.cell(widths[3], 8, str(row[Tension])[03], 1, 0, 'C')
            ...
            if col_btn1.button(📄 Exporter Carnet (PDF), type=primary)
                st.download_button(📥 Télécharger PDF, bytes(generate_pdf_cables()), fCables_{sanitize_text(st.session_state.projet['info']['nom'])}.pdf)
    ...
    # ---------------------------------------------------------
        ...
        if st.session_state.projet[tableaux]
            ...
            for i, nom_tab in enumerate(list(st.session_state.projet[tableaux].keys()))
                with onglets[i]
                    if st.button(f❌ Supprimer le tableau '{nom_tab}', key=fdel_{nom_tab})
                        del st.session_state.projet[tableaux][nom_tab]
                        st.rerun()

                    with st.form(fform_{i})
                    ...
                    if circuits
                        ...
                        st.metric(fTotal Absorbé ({nom_tab}), f{df_tab['P.Abs(W)'].sum()} W)
            ...
            with onglets[-1]
                ...
                if bilan_global
                    ...
                    c1_res.success(fPUISSANCE ACTIVE D'APPEL  {p_appel} W)
                    c2_res.info(fPUISSANCE APPARENTE (Abonnement)  {kva_estime} kVA)
                    ...
                    def generate_pdf_bilan()
                        ...
                        titre = sanitize_text(st.session_state.projet['info']['nom']).upper()
                        pdf.cell(190, 10, fBILAN DE PUISSANCE MULTI-TABLEAUX - {titre}, ln=True, align=C)
                        pdf.ln(5)
                        ...
                        for tab_name, circs in st.session_state.projet[tableaux].items()
                            ...
                            pdf.cell(190, 6, fSous-total absorbé pour {sanitize_text(tab_name)}  {sous_total} W, border='B', ln=True, align=R)
                            pdf.ln(4)

                        pdf.ln(5)
                        pdf.set_font(Helvetica, B, 12)
                        pdf.set_fill_color(255, 245, 230)
                        pdf.cell(190, 10, fPUISSANCE MAXIMALE D'APPEL (Ks={ks_global})  {p_appel} W, border=1, ln=True, align=C, fill=True)
                        pdf.cell(190, 10, fPUISSANCE APPARENTE ESTIMEE (Cos phi 0.8)  {kva_estime} kVA, border=1, ln=True, align=C)
                        return pdf.output()

                    if st.button(📄 Exporter Bilan Complet (PDF), type=primary)
                        st.download_button(📥 Télécharger Bilan PDF, bytes(generate_pdf_bilan()), fBilan_{sanitize_text(st.session_state.projet['info']['nom'])}.pdf)
    ...
    # ---------------------------------------------------------
        ...
        for cab in st.session_state.projet[cables]
            ...
            nomenclatures.append({Catégorie Câble, Désignation fCâble {nature} {type_c} - {cab['Section(mm2)']} mm2, Quantité cab[Long.(m)], Unité m, Prix Unitaire HT 15.0})
            nomenclatures.append({Catégorie Protection, Désignation fDisjoncteur {cab['Calibre(A)']}A, Quantité 1, Unité U, Prix Unitaire HT 80.0})

        for tab, circs in st.session_state.projet[tableaux].items()
            for c in circs
                cal_estime = 16 if c[P(W)] = 3500 else 20 if c[P(W)] = 4500 else 32
                nomenclatures.append({Catégorie Protection, Désignation fDisjoncteur Divisionnaire {cal_estime}A, Quantité 1, Unité U, Prix Unitaire HT 65.0})
        ...
        if not nomenclatures
            ...
            st.download_button(📊 Exporter vers Excel (.xlsx), data=to_excel(df_edited), file_name=fDevis_{sanitize_text(st.session_state.projet['info']['nom'])}.xlsx, type=primary)
    ...
    # ---------------------------------------------------------
        ...
        with onglets[0]
            ...
            with st.container(border=True)
                ...
                st.success(fBatterie condensateurs  {math.ceil(qc)} kVAR)
...
# ---------------------------------------------------------
        ...
        with tab_catalogue
            def charger_pdf(chemin_fichier)
                try
                    ...
                    return bLe catalogue PDF est en cours de mise a jour par l'equipe FC ELEC.

            col1, col2 = st.columns(2)
            with col1
                st.markdown(
                div style=border 1px solid #ddd; border-radius 10px; padding 20px; text-align center; margin-bottom 10px;
                    h3 style=color #0288d1;⚡ Études Électriques & NF C 15-100h3
                    pMaîtrisez les notes de calcul, le dimensionnement de câbles, et les logiciels (Caneco BT  AutoCAD).p
                div
                , unsafe_allow_html=True)
                st.download_button(📄 Télécharger le Plan (PDF), data=charger_pdf(plan_formation_etudes_electriques.pdf), file_name=Plan_Etudes_Electriques.pdf, mime=applicationpdf, use_container_width=True)
                st.write() 

            with col2
                st.markdown(
                div style=border 1px solid #ddd; border-radius 10px; padding 20px; text-align center; margin-bottom 10px;
                    h3 style=color #0288d1;☀️ Solaire Photovoltaïqueh3
                    pApprenez à dimensionner et concevoir des installations solaires autonomes et raccordées (PVsyst).p
                div
                , unsafe_allow_html=True)
                st.download_button(📄 Télécharger le Plan (PDF), data=charger_pdf(plan_formation_solaire.pdf), file_name=Plan_Solaire.pdf, mime=applicationpdf, use_container_width=True)
                st.write()
            ...
            with col3
                st.markdown(
                div style=border 1px solid #ddd; border-radius 10px; padding 20px; text-align center; margin-bottom 10px;
                    h3 style=color #0288d1;⚙️ Électricité Industrielleh3
                    pConception d'armoires, schémas de commande, variateurs de vitesse et automates programmables.p
                div
                , unsafe_allow_html=True)
                st.download_button(📄 Télécharger le Plan (PDF), data=charger_pdf(plan_formation_industrielle.pdf), file_name=Plan_Indus.pdf, mime=applicationpdf, use_container_width=True)

            with col4
                st.markdown(
                div style=border 1px solid #ddd; border-radius 10px; padding 20px; text-align center; margin-bottom 10px;
                    h3 style=color #0288d1;🚘 Bornes de Recharge (IRVE)h3
                    pNormes, dimensionnement et règles d'installation des Infrastructures de Recharge (Véhicules Électriques).p
                div
                , unsafe_allow_html=True)
                st.download_button(📄 Télécharger le Plan (PDF), data=charger_pdf(plan_formation_irve.pdf), file_name=Plan_IRVE.pdf, mime=applicationpdf, use_container_width=True)
        ...
        with tab_inscription
            ...
            div style=background linear-gradient(135deg, #01579b, #0288d1); padding 30px; border-radius 12px; text-align center; color white; margin-bottom 25px; box-shadow 0 8px 16px rgba(0,0,0,0.15);
                ...
                p style=margin 15px 0 0 0; font-size 1.1em; opacity 0.95;
                    Rejoignez l'élite de l'ingénierie électrique avec bFC ELEC ACADEMYb.br
            ...
            with st.form(formulaire_inscription)
                ...
                st.markdown(smalli Champs obligatoires pour valider le dossierismall, unsafe_allow_html=True)
                ...
                if soumis
                    if not nom_client or not email_client or not tel_client or not pays_client or sexe_client == Sélectionner
                        ...
                        texte_wa = (fBonjour FC ELEC !%0AJe souhaite sécuriser ma place pour la prochaine session.%0A%0A
                        ...
                        lien_wa = fhttpswa.me212674534264text={texte_wa}
                        ...
                        div style=background-color #e8f5e9; padding 25px; border-radius 8px; text-align center; border 2px solid #4CAF50; margin-top 15px;
                            h3 style=color #2e7d32; margin-top0;Dernière étape (Très important) ⏳h3
                ...
                if st.session_state.admin_connecte
                    ...
                    if not st.session_state.base_inscriptions
                        ...
                        st.download_button(
                            ...
                            file_name=fBase_Clients_FCELEC_{datetime.date.today().strftime('%d_%m_%Y')}.xlsx,
    ...
    # ---------------------------------------------------------
    st.markdown(brbr, unsafe_allow_html=True)
    ...
    div style=background-color #e3f2fd; padding 25px; border-radius 10px; text-align center; border-left 6px solid #0288d1; margin-bottom 30px; box-shadow 0 4px 6px rgba(0,0,0,0.05);
        ...
        p style=color #0277bd; font-size 1.1em; margin-bottom 20px;Rejoignez bFC ELEC ACADEMYb ! Des formations 100% pratiques et certifiantes pour les professionnels et étudiants.p
        a href=httpswa.me212674534264 target=_blank style=text-decoration none; background-color #FF4B4B; color white; padding 12px 25px; border-radius 5px; font-weight bold; transition 0.3s;
    ...
    with foot2
        st.markdown(#### 📱 Contact)
        st.write(WhatsApp  [+212 674-534264](httpswa.me212674534264))
        st.write(Email  [fcelec2.maroc@gmail.com](mailtofcelec2.maroc@gmail.com))
        
    with foot3
        st.markdown(#### 🌐 Réseaux)
        st.markdown([LinkedIn](httpswww.linkedin.comcompanyformation-et-consulting-en-electricite-fcelec)  [YouTube](httpswww.youtube.com@FCELECACADEMY))
        st.markdown([Facebook](httpswww.facebook.comprofile.phpid=61586577760070))
    ...
    st.markdown(
        ...
        div style=background-color #0e1117; padding 10px; border-radius 5px; text-align center; border-top 2px solid #FF4B4B; margin-top 20px;
            p style=color white; font-size 0.8em; margin 0;
                © 2026 bFC ELEC EXPERTb  Application gratuite développée pour l'ingénierie électrique.
            p
        div
        , 
        unsafe_allow_html=True
