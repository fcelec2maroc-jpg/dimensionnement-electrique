...
def to_excel(df):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name='Donnees_FCELEC')
...
if check_password():
            ...
            with st.form("ajout_cable"):
                ...
                if st.form_submit_button("Calculer et Ajouter"):
                    ...
                    st.session_state.projet["cables"].append({
                        "Repère": f"L{len(st.session_state.projet['cables'])+1}",
                        "P(W)": p_w, "Long.(m)": longueur, "Ib(A)": round(Ib, 1), "Section(mm2)": S_ret
                    })
    ...
    # ---------------------------------------------------------
    # MODULE 5 : CATALOGUE ET INSCRIPTION (VERSION EXCEL)
    # ---------------------------------------------------------
    elif menu == "📚 5. Catalogue des Formations":
        st.title("📚 FC ELEC ACADEMY : Formations & Inscription")
        tab_cat, tab_ins = st.tabs(["📖 Catalogue PDF", "📝 Inscription Directe"])

        with tab_cat:
            st.info("⚡ Études Électriques | ☀️ Solaire PV | ⚙️ Industrie | 🚘 IRVE")
            st.write("Retrouvez les programmes détaillés sur nos réseaux sociaux.")

        with tab_ins:
            st.markdown("""
            <div style="background: linear-gradient(135deg, #01579b, #0288d1); padding: 25px; border-radius: 12px; text-align: center; color: white; margin-bottom: 25px;">
                <h2 style="margin: 0;">🚀 Propulsez Votre Carrière !</h2>
                <p>Formations 100% pratiques • Attestation reconnue</p>
            </div>
            """, unsafe_allow_html=True)

            with st.form("form_inscription"):
                col1, col2 = st.columns(2)
                nom = col1.text_input("👤 Nom et Prénom *")
                email = col1.text_input("📧 E-mail *")
                tel = col2.text_input("📱 WhatsApp *")
                formation = col2.selectbox("🎓 Formation", ["Études Électriques", "Solaire PV", "Industrie", "IRVE"])
                soumis = st.form_submit_button("✅ JE RÉSERVE MA PLACE", type="primary", use_container_width=True)
            ...
            if soumis:
                if nom and email and tel:
                    ...
                    st.success(f"🎉 Merci {nom} ! Votre demande est enregistrée.")
                    msg = f"Bonjour FC ELEC, je m'inscris pour {formation}. Nom: {nom}"
        ...
        # --- ESPACE ADMINISTRATEUR (AVEC EXCEL) ---
        st.markdown("---")
        with st.expander("🔐 Accès Administrateur FC ELEC"):
            pwd = st.text_input("Mot de passe admin", type="password")
            if pwd == "FCELEC2026":
                st.markdown("#### 📋 Liste des Inscriptions")
                if st.session_state.base_inscriptions:
                    df_ins = pd.DataFrame(st.session_state.base_inscriptions)
                ...
                else: st.warning("Aucun inscrit pour le moment.")
